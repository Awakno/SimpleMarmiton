...
export async function POST(req: Request) {
  return new Promise(async (resolve) => {
    ...
    if (!contentType.startsWith('multipart/form-data')) {
      resolve(new Response(JSON.stringify({ error: 'Invalid content-type' }), { status: 400 }));
      return;
    }
    ...
    form.parse(nodeReq, async (err: any, fields: Fields, files: Files) => {
      if (err) {
        resolve(new Response(JSON.stringify({ error: 'Form parse error' }), { status: 500 }));
        return;
      }
      ...
      try {
        ...
        resolve(new Response(JSON.stringify({ error: 'Cloudinary upload failed' }), { status: 500 }));
      }
    });
  });
}
